...
run_create_infocenter_job() {
  ...
  SHA_256="$(./jenkins-cli.sh instances/technology.simrel console "${INFOCENTER_JENKINS_JOB}" "${LATEST_JOB_NUMBER}" -n 50 | grep "${RELEASE_NAME}@sha256" | head -n 1 | sed 's/.*sha256://g' | sed 's/ size:.*//g')"
